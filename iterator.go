...
//// DefaultIteratorOptions包含在迭代Badger键值存储时的默认选项。
...
////键值将以按字母顺序排序的方式返回。如果你要进行长时间的迭代，建议使用prefetch，以提高性能。
// Multiple Iterators:
...
func (txn *Txn) NewIterator(opt IteratorOptions) *Iterator {
	...
	// the prefix.
	// TODO: 如果设置了Prefix，则只挑选那些有前缀key的table。
	// 获取内存表列表
	tables, decr := txn.db.getMemTables()
	defer decr()
	//  给当前vlog对象一个迭代器引用计数表示当前迭代器
...
//// 如果有的话，Seek将寻找所提供的键。如果没有，它将寻求下一个 大于所提供的键的下一个最小的键，如果是向前迭代。
// 如果向后迭代，行为将被颠覆。
...
func (it *Iterator) Rewind() {
	//4. seek(nil) 将游标移动到最初始位置,遍历的，迭代性的调研底层迭代器的seek
	it.Seek(nil)
}
