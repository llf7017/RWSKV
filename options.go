...
type Options struct {
	...
	// DetectConflicts 决定tx是否需要进行冲突检测。当冲突检测被禁用时，事务可以以更高的速度被处理。
	DetectConflicts bool

	// NamespaceOffset specifies the offset from where the next 8 bytes contains the namespace.
	//// NamespaceOffset 指定接下来 8 个字节包含命名空间的偏移量。
	NamespaceOffset int
	...
	// 事务开始和提交的时间戳由终端用户管理。这只对建立在Badger之上的数据库（如Dgraph）有用。不建议大多数用户使用。
	managedTxns bool
