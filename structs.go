...
//// Encode 将Header编码为 []byte。 提供的 []byte 至少应为 5 个字节。 如果 out []byte 不足以容纳所有值，该函数将出现恐慌。
// 编码后的标头看起来像
// +------+----------+------------+--------------+-----------+
...
// Returns the number of bytes read.
// DecodeFrom从hashReader中读取Header。
// 返回读取的字节数。
...
//用于得出对应的kv对存在sstable中的大小，如果超过了valThreshold，那么就kV分离，否则就不分离，如果KV分离，那么长度就需不算value的长度，但需要算指针的大小12
...
//// NewEntry用args中传递的key和value创建一个新条目。这个新创建的条目可以通过调用 txn.SetEntry() 在事务中设置。
//Entry 的所有其他属性可以通过调用 WithMeta, WithDiscard, WithTTL 方法来设置。
//这个函数使用键和值的引用，因此用户在TX结束前不能修改key和value。
