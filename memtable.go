...
func (mt *memTable) Put(key []byte, value y.ValueStruct) error {
	...
	/// 只有当badger在内存模式下运行时，wal才为零，我们不需要wal。
	if mt.wal != nil {
...
// encodeEntry 将对 buf 的入口进行编码
// layout of entry
// +--------+-----+-------+-------+
// | header | key | value | crc32 |
// +--------+-----+-------+-------+
//  kv数据在文件中的编码是 head，key，value，crc
func (lf *logFile) encodeEntry(buf *bytes.Buffer, e *Entry, offset uint32) (int, error) {
	...
	if lf.encryptionEnabled() {
		...
		// TODO: 不需要分配字节。我们可以一个一个地计算加密的buf，因为我们使用的是AES加密的ctr模式。
		// 顺序不会改变。需要一些 XORBlock的重构，它将像流密码一样工作。
...
func (lf *logFile) doneWriting(offset uint32) error {
	...
	//
	// 之前我们在lf.lock上获取了一个锁，因为我们把文件描述符重新开放为只读而使其失效。
	//现在，我们并没有使fd失效，而是取消了它的映射，截断了它，然后重新映射。这就产生了一个窗口，因为mmap不再有效，我们会出现segfaults。
	//不再有效，而有人可能正在读取它。因此，我们在这里又需要一个锁。
	lf.lock.Lock()
	...
	//
	// 以前我们习惯于在文件被写入后关闭它，然后以只读模式重新打开它。我们不再以只读模式打开文件。我们保持所有的vlog文件以读写模式打开。
	return nil
}

// iterate iterates over log file. It doesn't not allocate new memory for every kv pair.
// Therefore, the kv pair is only valid for the duration of fn call.
// iterate 遍历日志文件。它不会为每个kv对分配新的内存。
// 因此，kv对只在fn调用期间有效。
...
loop:
	for {
		...
		// zero'ed it out.
		// 我们还没有到达文件的末尾，但我们读到的条目是零。发生这种情况是因为我们截断了文件并将其清零。
		case err == io.EOF:
			break loop
...
func (lf *logFile) open(path string, flags int, fsize int64) error {
	...
	if lf.size < vlogHeaderSize {
		...
		//每个vlog文件应该至少有vlogHeaderSize。如果它小于vlogHeaderSize，那么它一定被损坏了。
		//但这里不需要处理。log replayer会截断并引导日志文件。所以忽略这里。
		return nil
	}
