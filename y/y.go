...
//os.O_EXCL与 O_CREATE 标志组合起来调用 open，确保指定的文件由 open 的调用者创建，否则返回错误。
//即，如果进程 A 用 O_CREATE 和 O_EXCL 标志来调用 open，期望创建一个指定的文件 file1，
//如果 file1 不存在，则 open 成功返回且创建 file1，如果 file1 已经存在了（即不是由进程 A 创建的），那么 open 返回错误。
...
//// Do应该在worker开始工作之前被他们调用。如果已经有最大数量的worker在工作，它就会阻塞。如果它检测到之前的Done工作者的错误，它将会返回。
