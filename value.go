...
const (
	...
	bitFinTxn byte = 1 << 7 // Set if the entry is to indicate end of txn in value log.如果该entry在数值日志中表示txn的结束，则进行设置。
...
//// hashReader实现了io.Reader,io.ByteReader接口。它还记录了读取的字节数。hashReader将它从r读取的内容写入h（散列）。
...
//// Entry从提供的Reader中读取一个条目。它还验证每个被读取条目的 checksum，失败时返回错误。
...
//对value log进行初始化，在进行compaction之前必须完成
...
func (vlog *valueLog) open(db *DB) error {
	...
	//获得所有vlog的filesMap信息，key:fileid value:*logFile
	...
	// log open.
	// 现在我们可以读取最新的V-LOG文件，看看它是否需要截断。从技术上讲，我们可以在所有的value log上做这件事，但这意味着会减缓value log打开速度
...
//  检查写入的vlaue是否合法,是否足够写入4G大小的文件，因为值指针是uin32
func (vlog *valueLog) validateWrites(reqs []*request) error {
	...
	for _, req := range reqs {
		...
		if estimatedVlogOffset >= uint64(vlog.opt.ValueLogFileSize) {
			...
			//如果估计的偏移量大于或等于最大 vlog 大小，我们将创建一个新的 vlog 文件。 因此，重置 vlogOffset。
			vlogOffset = 0
			continue
		}
...
//estimateRequestSize 返回给定请求需要写入的大小。
...
func (vlog *valueLog) write(reqs []*request) error {
	...
	for i := range reqs {
		...
		for j := range b.Entries {
			...
			//用来判断value的大小是否过小，如果很小那么直接存入sstable,无需指针信息
			...
			//我们不应该在 vlog 文件中存储事务标记，因为它永远不会包含事务中的所有条目。
			//如果我们存储带有事务标记的条目，那么 value GC 将无法迭代整个 vlog 文件。
			//但是，我们仍然希望 memTable WAL 的条目保持不变。 因此，将元数据存储在一个临时变量中，并在写入Value log后重新分配它。
			tmpMeta := e.meta
...
func (vlog *valueLog) getFileRLocked(vp valuePointer) (*logFile, error) {
	...
	// 在只读模式下，我们不需要检查可写偏移量，因为我们没有写任何东西。
	// 而且，这个偏移量在只读模式下是不设置的。
...
func (vlog *valueLog) Read(vp valuePointer, _ *y.Slice) ([]byte, func(), error) {
	...
	// 日志文件被锁定了，所以要决定是立即锁定还是让调用者解锁，在调用者使用它之后。
...
// logFile unlocking.
// readValueBytes 返回vlog entry slice并读取锁定的日志文件。调用者应该处理好 日志文件的解锁。
...
LOOP:
	...
	//	//MaxDiscard如果没有任何丢弃的数据，将返回fid=0。vlog文件从1开始。
	if fid == 0 {
	...
	//// 这个文件被删除了，但是由于压缩，它的丢弃统计量增加了。该文件 并不存在，所以我们不需要做任何事情。跳过它并重试。
