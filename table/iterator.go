...
func (itr *blockIterator) setIdx(i int) {
	...
	// 基于offset进行编解码，找到对应的key，value字节数据
	var endOffset int
	...
	// Header包含key重叠的长度和与基础钥匙相比的差异。如果在这之前的钥匙有相同或更好的钥匙重合度，
	// 我们可以避免将该部分复制到 itr.key。但是，如果重合度较小，我们可以只复制那部分。
...
// NewConcatIterator创建一个新的串联迭代器，用于level 1-7
