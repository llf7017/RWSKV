...
type Table struct {
	...
	*z.MmapFile
	...
	tableSize int // Initialized in OpenTable, using fd.Stat().在OpenTable中初始化，使用fd.Stat()。

	_index *fb.TableIndex // Nil if encryption is enabled. Use fetchIndex to access.如果启用了加密功能，则为零。使用fetchIndex来访问。
	_cheap *cheapIndex
	ref    int32 // For file garbage collection. Atomic.用于文件垃圾收集
	...
	IsInmemory bool // Set to true if the table is on level 0 and opened in memory.如果table在第0层并在内存中打开，则设置为true
	opt        *Options
}
...
type cheapIndex struct {
	...
	OffsetsLength     int //该table的block数量
}
...
// OnDiskSize返回存储在此table中的键值的总大小（包括value-log上占用的磁盘空间）。
...
type block struct {
	...
	freeMe            bool     // used to determine if the blocked should be reused.用来确定是否应该重复使用被封锁的数据。
	ref               int32
}
...
//// OpenTable 假设文件只有一个表，并打开它。在函数进入时取得fd的所有权。返回一个有一个引用计数的表（递减可能会删除该文件！）。
//-- 考虑用t.Close()代替）。fd必须是可写的，因为我们在删除前会对它调用Truncate。表的所有块的校验和是根据chkMode的值来验证的。
...
// first block offset
// initIndex 读取索引，填充必要的表字段，并返回第一个块的偏移。
...
// block函数返回一个新块。 每个块都有一个 ref，当 ref 变为零时，存储在块中的字节片将被重用。 调用者应该通过调用 block.decrRef() 来释放块。
func (t *Table) block(idx int, useCache bool) (*block, error) {
	...
	if useCache && t.opt.BlockCache != nil {
		...
		//// 将该block插入到cache中，如果我们不能在缓存中插入该块，则减少该块的引用。cache是一个map结构
...
// DoesNotHave在Table没有key哈希值的情况下返回true。
// 它做了一个Bloom filter的查找。
...
// for the table.
//// shouldDecrypt 告诉是否要解密。 仅当表的数据密钥存在时，我们才解密。
...
// 解密给定的数据。 只有在检查了 shouldDecrypt 之后才应该调用它。
...
//ParseFileID从一个文件名中读出文件id
func ParseFileID(name string) (uint64, bool) {
	//filepath.Base()获取 path 中最后一个分隔符之后的部分（不包含分隔符）
	name = filepath.Base(name)
	//HasSuffix 测试字符串name是否以后缀filesuffix结尾。
