...
const (
	...
	//当一个block被加密后，它的长度会增加。我们添加256字节的填充来处理块大小增加的情况。这是一个近似的数字。
	padding = 256
)

type header struct {
	overlap uint16 // Overlap with base key.//与base key重合部分的长度
	diff    uint16 // Length of the diff.   //与base key不一样的部分长度
}
...
type Builder struct {
	...
	//通常是几十或几百兆，针对一个单一的是stable。
	// Allocator通过在更大的块中分配内存来分摊小块分配的成本。 在内部，它使用z.Calloc来分配内存。一旦被分配，内存就不会被移动，
	//所以使用被分配的字节不安全地投到Go结构指针上是安全的。维护一个自由列表是很慢的。
	//相反，Allocator只分配内存，其想法是最后我们只需释放整个Allocator。
	alloc            *z.Allocator
	curBlock         *bblock
